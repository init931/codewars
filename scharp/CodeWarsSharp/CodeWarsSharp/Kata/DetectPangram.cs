using System;
using System.Collections.Generic;
using System.Linq;

namespace CodeWarsSharp.Kata {
    public partial class Kata {
        /// <summary>
        /// https://www.codewars.com/kata/545cedaa9943f7fe7b000048/train/csharp
        /// A pangram is a sentence that contains every single letter of the alphabet at least once.
        /// For example, the sentence "The quick brown fox jumps over the lazy dog" is a pangram,
        /// because it uses the letters A-Z at least once (case is irrelevant).
        /// Given a string, detect whether or not it is a pangram.
        /// Return True if it is, False if not.Ignore numbers and punctuation.
        /// </summary>
        public static bool DetectPangram(string str) {
            var letters = new List<char>();
            foreach (char ch in str.ToLower()) {
                if (Char.IsLetter(ch) && !letters.Contains(ch)) {
                    letters.Add(ch);
                }
            }
            return letters.Count() == 26;
        }

        public static bool DetectPangramMy_GroupBy(string str) {
            return str.ToUpper().GroupBy(x => x).Where(x => Char.IsLetter(x.Key)).Count() == 26;
        }
    }
}
